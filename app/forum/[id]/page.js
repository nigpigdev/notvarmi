...
export default function PostDetail() {
    ...
    useEffect(() => {
        const checkSavedStatus = async () => {
            ...
            try {
                const res = await fetch(`/api/forum/save?postId=${params.id}`);
                if (res.ok) {
                    const data = await res.json();
    ...
    const handleSavePost = async () => {
        ...
        try {
            if (isSaved) {
                await fetch(`/api/forum/save?postId=${params.id}`, { method: 'DELETE' });
                setIsSaved(false);
                    ...
                    <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.25rem' }}>
                                {session ? (
                                    <Link href={`/profile/${post.author.username}`} style={{ fontWeight: '700', color: 'var(--text)', fontSize: '1rem', textDecoration: 'none' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                            ...
                                            <div>
                                                <div style={{ fontWeight: '600', color: 'var(--text)', fontSize: '0.95rem' }}>
                                                    {session ? (
                                                        <Link href={`/profile/${reply.author.username}`} style={{ color: 'var(--text)', textDecoration: 'none' }}>
                                                            {reply.author.name}
                                                        </Link>
                                                    ) : (
                                                        reply.author.name
