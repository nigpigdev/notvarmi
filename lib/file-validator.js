...
// Filename patterns that are explicitly blocked (partial match)
const BLOCKED_FILENAME_PATTERNS = [
    ...
    'xmr',
    'hashrate',
    '.sys', // potential system file masking
];
...
export function validateFileType(file, buffer, allowedMimeTypes = null) {
    ...
    // 0. Check for blocked filename patterns
    const lowName = filename.toLowerCase();
    for (const pattern of BLOCKED_FILENAME_PATTERNS) {
        if (lowName.includes(pattern)) {
            return {
                valid: false,
                error: 'Dosya adı güvenlik politikası gereği engellendi.'
            };
        }
    }

    // 0.1 Check for hidden files
    if (filename.startsWith('.')) {
        return {
            valid: false,
            error: 'Gizli dosyaların yüklenmesi yasaktır.'
        };
    }
    ...
    // 6. Special check for text/plain to prevent binary files being uploaded as text
    if (mimeType === 'text/plain' && buffer) {
        ...
        for (let i = 0; i < checkLen; i++) {
            if (buffer[i] === 0) { // Null byte is a strong indicator of binary
                return {
                    valid: false,
                    error: 'Metin dosyası ikili (binary) veri içeremez.'
                };
            }
            // Optional: check for other control characters if needed, but null byte is usually sufficient to catch executables renamed as txt
        }
    }
...
export function sanitizeFilename(filename) {
    ...
    return baseName
        .replace(/[^a-zA-Z0-9._-]/g, '_')  // Replace EVERYTHING except basic alphanumeric/dot/underscore/dash
        .replace(/\.{2,}/g, '.')            // Prevent path traversal attempts via multiple dots
        .replace(/^\./, 'dot_')              // Don't allow leading dots (hidden files)
        .substring(0, 100);                  // Limit length significantly
}
...
export { DANGEROUS_EXTENSIONS, SAFE_FILE_TYPES, BLOCKED_FILENAME_PATTERNS };
